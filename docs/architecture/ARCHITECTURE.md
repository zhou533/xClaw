...
│   │   │   │   ├── policy.rs     # SessionPolicy（轮换策略配置）
│   │   │   │   ├── expiry.rs     # is_expired 过期判断（pub(crate)）
│   │   │   │   └── time_util.rs  # 时间工具函数（pub(crate)）
...
同一 `SessionKey` 下的 session 不会无限存续。`SessionPolicy` 定义轮换规则，内聚于 session 模块内部：

```rust
/// 会话轮换策略（xclaw-memory/src/session/policy.rs）
pub struct SessionPolicy {
    pub reset_at_hour: u8,        // 每日重置时间（UTC），默认 4
    pub idle_minutes: Option<u64>, // 空闲超时（分钟），None 时不启用
}
```

轮换触发条件（OR 关系，任一满足即创建新 SessionId）：
- **每日重置 (Daily Mode)**：`updated_at` 在今日 `reset_at_hour` 之前 → 过期
- **空闲超时 (Idle Mode)**：距 `updated_at` 超过 `idle_minutes` → 过期
- **用户主动重置**：调用 `reset_session` 强制创建新 session

过期判断由纯函数 `is_expired(updated_at, now_epoch_secs, policy)` 实现（`pub(crate)`），时间作为参数注入，不读系统时钟。轮换逻辑在 `FsSessionStore::get_or_create` 实现内部处理，`SessionStore` trait 签名不变，调用方零改动。旧 session 保留不删除（历史可追溯）。同一 `SessionKey` 可对应多条 `SessionEntry`，`get_by_key` 返回最新创建的匹配。
...
pub trait SessionStore: Send + Sync {
    ...
    fn reset_session(&self, role: &RoleId, key: &SessionKey)
        -> impl Future<Output = Result<SessionEntry, MemoryError>> + Send;
...
> 详细设计参见 [session-system.md](session-system.md)、[session-id-renewal.md](session-id-renewal.md) 和 [ADR-006](adr/ADR-006-session-system.md)。
